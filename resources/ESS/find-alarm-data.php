...
if (($_SESSION['page_admin']) == 0) {
  ...
  $sqlAlarmDataExterior = $mysqli->query("SELECT * FROM alarmData
    where (unitName = '412 SFS' AND (status <> 'completed' AND accountedFor <>'Y' AND alarmLocationType = 'Exterior') and
    (alarmTypeSubmit = '340') and (reportedTime BETWEEN now() - INTERVAL 1 DAY AND now() + INTERVAL 1 DAY ))


    OR
    (unitName = '412 SFS' AND status <> 'completed' AND alarmLocationType = 'Exterior' and alarmTypeSubmit = '781A')


    ORDER BY alarmTypeSubmit DESC, accountName asc, buildingNumber ASC, sensorKind ASC, fossZone ASC, alarmLocationType ASC, reportedTime ASC") or die(mysqli_errno($mysqli));
    ...
    $sqlAlarmDataInterior = $mysqli->query("SELECT * FROM alarmData
      ...
      $findCompleted = $mysqli->query("SELECT * FROM alarmData  where unitName = '$unitName' AND status = 'completed'");
      ...
      //exterior alarms
      //
      if ($sqlAlarmDataExterior->num_rows > 0) {
        // output data of each row

        echo "<br><h3 style='text-align: center'>Exterior</h3><table>";
        echo "<table id='ExteriorAlarmTables' class='table table-bordered table-hover'>
        <thead>
        <tr>
        <th>Date</th>
        <th>Form Type</th>
        ...
        while ($row = $sqlAlarmDataExterior->fetch_assoc()) {
    ...
    //Completed alarms
    //
    if ($findCompleted->num_rows > 0) {
      // output data of each row

      echo "<br><h3 style='text-align: center'>Completed Alarm Data</h3><table>";
      echo "<table id='AdminViewCompletedAlarmTables' class='table table-bordered table-hover'>
      <thead>
      <tr>
      <th>Date</th>
      <th>Form Type</th>
      ...
      while ($row = $findCompleted->fetch_assoc()) {
