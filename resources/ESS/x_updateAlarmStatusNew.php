...
$resultFindUser = mysqli_query($mysqli, $finduser);
...
$updateItemId = mysqli_real_escape_string($mysqli, $_GET['id']);

$now = date('Y-m-d');

$findSubmitter = "SELECT * FROM alarmData where id = '$updateItemId' and unitName ='$unitName'";

$resultsFindSubmitter = mysqli_query($mysqli, $findSubmitter);
...
if((($_SESSION['page_admin']) == 'ESSAdmin') OR (($_SESSION['page_admin']) == 'matt') OR ($recallSubmittedBy === $recallUser)) {

    if (isset($_GET['id'])) {

       if((($_SESSION['page_admin']) == 'ESSAdmin') OR (($_SESSION['page_admin']) == 'matt')){

                $updateFormType = mysqli_real_escape_string($mysqli, $_POST['inputFormType']);
            $updateAccountName = mysqli_real_escape_string($mysqli, $_POST['inputAccountName']);
           if(!$_POST['inputAccessPoint'] == ''){

               $updateAccessPoint = mysqli_real_escape_string($mysqli, $_POST['inputAccessPoint']);
           }else{
               $updateAccessPoint = '0';
           }
            $updateSensorLocation= mysqli_real_escape_string($mysqli, $_POST['inputSensorLocation']);
            $updateFindings= mysqli_real_escape_string($mysqli, $_POST['inputFindings']);
            $updateDescription= mysqli_real_escape_string($mysqli, $_POST['inputDescription']);
            ...
            $updateBuildingNumber= mysqli_real_escape_string($mysqli, $_POST['inputBuildingNumber']);
            $updateInspectedDate = mysqli_real_escape_string($mysqli, $_POST['dateInspected']);
            $updateInspectedBy = mysqli_real_escape_string($mysqli, $_POST['inspectedBy']);
            $updateNotes= mysqli_real_escape_string($mysqli, $_POST['noteBox']);
            $updateCorrectedBy= mysqli_real_escape_string($mysqli, $_POST['inputCorrectedBy']);
            $updateStatus = mysqli_real_escape_string($mysqli, $_POST['alarmUpdate']);
       ...
       if (isset($_POST['update'])){
            ...
            $updateAlarmStatus = "UPDATE alarmData SET status = '$updateStatus', notes = '$updateNotes', inspectedBy = '$updateInspectedBy', dateCorrected = '$updateInspectedDate',
                ...
                $resultsUpdateAlarmStatus = mysqli_query($mysqli, $updateAlarmStatus) or die(mysqli_error($mysqli));
...
<?php
        ...
        if ($resultSelectAlarmUpdate > 0) {
            ...
            if((($_SESSION['page_admin']) == 'ESSAdmin') OR (($_SESSION['page_admin']) == 'matt')){
                ...
                if ($accessPoint == '' or NULL) {

                    echo "<td><input type='text' class='form-control' id='inputAccessPoint' name='inputAccessPoint' value='' placeholder='Access Point'></td>";
                } else {
                    echo "<td><input type='text' class='form-control' id='inputAccessPoint' name='inputAccessPoint' value='".$accessPoint."' placeholder=''></td>";
                }
                ...
                echo "<td><select class='form-control' id = 'inputFindings' name = 'inputFindings' title = 'inputFindings'>
                   <option value = 'Other'"; if ($findings == 'Other') {echo 'selected';} echo " >Other</option >
                   <option value = 'False'"; if ($findings == 'False') {echo 'selected';}echo " >False</option >
                   <option value = 'Nuisance'"; if ($findings == 'Nuisance') {echo 'selected';}echo ">Nuisance</option ></select ></td>";
