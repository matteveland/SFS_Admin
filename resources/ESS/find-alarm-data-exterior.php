...
if (($_SESSION['page_admin']) == 0) {
    ...
    $sqlAlarmDataexterior = $mysqli->query("SELECT * FROM alarmData
      where (unitName = '$unitName' AND (status <> 'completed' AND accountedFor <>'Y' AND alarmLocationType = 'exterior' and (alarmTypeSubmit = '340') and (reportedTime BETWEEN now() - INTERVAL 30 DAY AND now() + INTERVAL 1 DAY )))

      OR (unitName = '$unitName' AND status <> 'completed' AND alarmLocationType = 'exterior' and (alarmTypeSubmit = '781A'))
